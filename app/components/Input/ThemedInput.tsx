...
import { SpacingProps, BoxProps, Box } from "components/Box";
import { Colors } from "app/config/styles";
interface TextInputBaseProps extends RNTextInputProps, Omit<BoxProps, "style"> {
  /**
...
const StyledRoot = (props: RootProps) => {
  ...
  return <Box {...viewProps}>{children}</Box>;
};
...
const StyledInput = (props: InputProps) => {
  ...
  const {
    ...
    ? {
        borderRadius: 0, //theme.controlBorderRadius[size],
    ...
    : {
        borderRadius: 0, //theme.controlBorderRadius.medium,
        minHeight: size,
  ...
  return (
    ...
    <RNTextInput
      ...
      editable={!isDisabled}
      placeholderTextColor={placeholderTextColor || Colors.darkA300}
...
const StyledRightIcon = (props: StyledIconProps) => {
  ...
  return (
    <TouchableOpacity onPress={handlePress}>
      <MaterialIcon
        name="close-circle"
        color={theme.colors.background.greyDark}
        size={15}
      />
    </TouchableOpacity>
  );
