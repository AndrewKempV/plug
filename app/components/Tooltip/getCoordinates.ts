...
const getTooltipCoordinate = (
  ...
  // Shift the coordinates in the direction of the quadrant.
  const directionCorrection = [[-1, -1], [1, -1], [1, 1], [-1, 1]];
  const shiftReferencePoint = [
  ...
  const withPointerOffsetY = withPointer
    ? 10 * directionCorrection[qIndex][1]
    : 0;
  const withPointerOffsetX = withPointer
    ? center[0] - 18 * directionCorrection[qIndex][0]
    : center[0];

  const newX =
    withPointerOffsetX +
    (dX * directionCorrection[qIndex][0] + shiftReferencePoint[qIndex][0]);

  return {
    x: constraintX(newX, qIndex, center[0], ScreenWidth, tooltipWidth),
    y:
      center[1] +
      (dY * directionCorrection[qIndex][1] + shiftReferencePoint[qIndex][1]) +
      withPointerOffsetY
  };
};
